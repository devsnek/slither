...
use crate::value::{new_builtin_function, new_iter_result, new_object, ObjectKey, Value};
...
fn next(agent: &Agent, ctx: &ExecutionContext, args: Vec<Value>) -> Result<Value, Value> {
    ...
    if let Value::WrappedContext(context, _) = this.get_slot("generator context") {
        ...
        if context.borrow_mut().evaluator.is_none() {
            new_iter_result(agent, Value::Null, true)
        } else {
            ...
            match evaluator.run(agent) {
                Ok(r) => match r {
                    Ok(v) => new_iter_result(agent, v, true),
                    Err(e) => Err(e),
                },
                Err(c) => {
                    ...
                    new_iter_result(agent, value, false)
                }
            }
        }
    } else {
