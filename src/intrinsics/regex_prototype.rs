...
use crate::value::{new_builtin_function, new_error, new_array, new_object, ObjectKey, ObjectKind, Value};
...
fn match_(agent: &Agent, ctx: &ExecutionContext, args: Vec<Value>) -> Result<Value, Value> {
    ...
    match this {
        Value::Object(o) => if let ObjectKind::Regex(re) = &o.kind {
            let mut args = args;
                    ...
                    match re.captures(s.as_str()) {
                        Some(captures) => {
                            ...
                            for name in re.capture_names() {
                                match name {
                                    Some(s) => {
                                        o.set(&ObjectKey::from(s), Value::String(captures.name(s).unwrap().as_str().to_string()))?;
                                    }
                                    None => {
                                        o.set(&ObjectKey::from(i), Value::String(captures.get(i).unwrap().as_str().to_string()))?;
                                        i += 1;
                                    }
                                }
                            }
...
pub fn create_regex_prototype(agent: &Agent) -> Value {
    ...
    proto
        .set(
            &ObjectKey::from("test"),
            new_builtin_function(agent, test),
        )
        .unwrap();
