...
setlocal indentexpr=GetSlitherIndent()
setlocal autoindent nolisp nosmartindent
setlocal indentkeys+=0],0)
" Testable with something like:
" vim  -eNs "+filetype plugin indent on" "+syntax on" "+set ft=slither" \
...
" au! Filetype slither let b:syng_str = '^\%(.*template\)\@!.*string\|special'
" au! Filetype slither let b:syng_strcom = '^\%(.*template\)\@!.*string\|comment\|regex\|special\|doc'
...
" GetSlitherIndent does to find the containing [[{(] (side-effects)
function s:SkipFunc()
  if s:top_col == 1
    throw 'out of bounds'
  elseif s:check_in
...
let s:opfirst = '^' . get(g:,'slither_opfirst',
      \ '\C\%([<>=,.?^%|/&]\|\([-:+]\)\1\@!\|\*\+\|!=\|in\%(stanceof\)\=\>\)')
let s:continuation = get(g:,'slither_continuation',
      \ '\C\%([<=,.~!?/*^%|&:]\|+\@<!+\|-\@<!-\|=\@<!>\|\<\%(typeof\|new\|delete\|void\|await\)\)') . '$'
...
function GetSlitherIndent()
  ...
  elseif idx == -1 && s:LookingAt() == '(' && &cino =~ '(' &&
    ...
    if !pval
      ...
      if search('\m'.get(g:,'slither_indent_W_pat','\S'),'W',num)
